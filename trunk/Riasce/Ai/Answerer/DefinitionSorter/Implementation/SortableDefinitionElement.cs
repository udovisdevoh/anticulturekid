...
namespace AntiCulture.Kid
{
    class SortableDefinitionElement : IComparable
    {
        ...
        public int CompareTo(object obj)
        {
            ...
            if (sortingMode == DefinitionSorter.SORT_BY_LOOSE_COMPLEMENT_CARDINALITY)
            {
                ...
                {
                    int verb1Key = DefinitionSorter.customOrder.Count;
                    int verb2Key = DefinitionSorter.customOrder.Count;

                    if (DefinitionSorter.customOrder.TryGetValue(this.verbName, out verb1Key)) { }
                    if (DefinitionSorter.customOrder.TryGetValue(otherElement.verbName, out verb2Key)) { }

                    if (verb1Key == DefinitionSorter.customOrder.Count && verb2Key == DefinitionSorter.customOrder.Count)
                    {
            ...
            else if (sortingMode == DefinitionSorter.SORT_BY_VERB_NAME)
            {
            ...
            else if (sortingMode == DefinitionSorter.SORT_BY_CUSTOM_ORDER)
            {
                int verb1Key = DefinitionSorter.customOrder.Count;
                int verb2Key = DefinitionSorter.customOrder.Count;
            ...
            else if (sortingMode == DefinitionSorter.SORT_BY_PROOF_LENGTH)
            {
